...
using Winapi::WinUser::Input::MouseInput;
...
namespace PixelsPerDegreeCalculator
{
    ...
    static const Dword move_x_vkcode = 39;       // right arrow
    static const Dword test_x_vkcode = 37;       // left arrow
    static const Dword move_y_vkcode = 40;       // down arrow
    static const Dword test_y_vkcode = 38;       // up arrow
    static const Dword print_vkcode = 80;        // p
    static const Dword zero_vkcode = 48;         // 0
    static const Dword inverse_test_vkcode = 73; // i
    ...
    static void run(const Args args)
    {
        ...
        Decider decider
        {
            state,
            {
                Decision<KeyEvent>(
                    ...
                    {
                        return event.type == KeyEventType::Down && event.info->vkCode == move_x_vkcode;
                    },
                    [&args](State& state, const KeyEvent& event)
                    {
                        ...
                        state.get<Long>(x_total_px_count) += delta;
                ...
                Decision<KeyEvent>(
                    [](const State& state, const KeyEvent& event)
                    {
                        return event.type == KeyEventType::Down && event.info->vkCode == move_y_vkcode;
                    },
                    [&args](State& state, const KeyEvent& event)
                    {
                        ...
                        state.get<Long>(y_total_px_count) += delta;
                        state.get<size_t>(total_mouse_move_count)++;
                    }
                ),
                Decision<KeyEvent>(
        ...
        KVMSHook::initialize(
            decider.get_key_event_handler(),
            nullptr
        );

        input_listener = new InputListener(KVMSHook::get_hook_proc());
